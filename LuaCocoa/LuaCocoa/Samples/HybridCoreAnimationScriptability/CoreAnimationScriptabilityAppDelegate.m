...
//	NSString* error_string = [luaCocoa pcallLuaFunction:"OnLoadFinished" withSignature:""];
	NSString* error_string = [luaCocoa pcallLuaFunction:"OnLoadFinished" withSignature:"@@", self.animatableLayer, self];
	if(nil != error_string)
	{
...
//	CGColorSpaceRelease(rgb_color_space);
	...
	CATransform3D layer_transform = CATransform3DIdentity;
	layer_transform.m34 = -1.0 / 20000.0;
	[[self.theWindow.contentView layer] setSublayerTransform:layer_transform];
//	[[self.theWindow.contentView layer] setTransform:layer_transform];
}
...
{
	NSString* error_string = [luaCocoa pcallLuaFunction:"OnAction6" withSignature:"@@", self.animatableLayer, self];
	if(nil != error_string)
	{
		[self promptUserToOpenLuaFileAtError:error_string];
	}
...
{
	NSString* error_string = [luaCocoa pcallLuaFunction:"OnAction7" withSignature:"@@", self.animatableLayer, self];
	if(nil != error_string)
	{
		[self promptUserToOpenLuaFileAtError:error_string];
	}
